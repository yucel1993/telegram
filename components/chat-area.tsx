        ...
        <div className="relative group">
          <div
            className={cn(
              ...
              "p-3 rounded-lg",
              "inline-block", // key change: makes the bubble grow with content width
              "break-words", // breaks long words nicely
              "max-w-[80%]", // allows up to 80% width of container
              isCurrentUser
                ? `bg-blue-500 pr-8 text-white ${
          ...
          >
            ...
            {message.fileAttachment && (
              <div
                className={`mt-2 ${
                  isCurrentUser ? "bg-blue-600" : "bg-gray-50"
          ...
          {reactionGroups.length > 0 && (
            <div
              className={`flex flex-wrap gap-1 mt-1 ${
                isCurrentUser ? "justify-end mr-12" : "justify-start"
              }`}
            >
              <TooltipProvider>
    ...
    async function fetchMessages() {
      try {
        ...
        if (data.messages && Array.isArray(data.messages)) {
          ...
          if (messagesJson !== lastMessagesJsonRef.current) {
            ...
            const shouldAutoScroll =
              autoScroll && data.messages.length > lastMessageCountRef.current;
        ...
        if (data.isGroup) {
          ...
          const isMember = data.participants
            ? data.participants.some((p: any) => p._id === userId)
            : false;
        ...
        if (
          data.messages &&
          data.messages.some((m: any) => !m.read && m.sender !== userId)
        ) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-4 pt-10 bg-gray-50 relative"
      >
        <div className="space-y-4">
          {messages.length === 0 ? (
            ...
            messages.map((message, index) => {
              ...
              const isCurrentUser =
                message.sender === userId || message.isUserMessage;
          ...
          {isGroup && !isGroupMember ? (
            ...
            >
              ...
              <Button
                type="submit"
                disabled={
                  (messageText.trim() === "" && !fileAttachment) || sending
                }
                size="icon"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              added back by a member to rejoin.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>Cancel</AlertDialogCancel>
